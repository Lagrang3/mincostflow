...
int main()
{
    ...
    while(1)
    {
        ...
        for(int e=0;e<N_edges;++e)
        {
            ...
            weights.at(arc.x) = w;
            weights.at(arc2.x) = solver.INFINITY;
        }
        ...
        solver.solve(Graph,s_node,weights,
            [&weights,&solver](arc_pos_t arc){return weights.at(arc.x)<solver.INFINITY;});
        ...
        for(int v;Q--;)
        {
            ...
            if(Graph.is_valid(pos) && solver.distance.at(pos.x)<solver.INFINITY)
            {
